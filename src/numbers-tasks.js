...
function getAverage(value1, value2) {
  ...
  if (!Number.isFinite(sum)) {
    sum = sum > 0 ? Number.MAX_VALUE : -Number.MAX_VALUE;
  }
  return sum;
}
 ...
 */ // d=√((x2 – x1)² + (y2 – y1)²)  Pythagoras theorem, hypotenuse
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const findMagnitude = (a, b) => Math.sqrt(a * a + b * b);
  const angle = Math.acos(
    (dotProduct / findMagnitude(x1, y1)) * findMagnitude(x2, y2)
  );
  return angle;
}
 ...
 * no-unused-vars && no-param-reassign
 *
 * @param {number} num
 * @return {boolean}
 *
...
function numberToStringInBase(number, base) {
  ...
  const isNum = typeof number.valueOf() === 'number';
  const isValidBase = base >= 2 && base <= 16;
  return isNum && isValidBase ? number.toString(base) : 'error';
}
...
function toExponential(number, fractionDigits) {
  const isNum = typeof number.valueOf() === 'number';
  const isValidRange = fractionDigits >= 0 && fractionDigits <= 100;
  return isNum && isValidRange ? number.toExponential(fractionDigits) : 'error';
}
...
function toFixed(number, fractionDigits) {
  const isNum = typeof number.valueOf() === 'number';
  const isValidRange = fractionDigits >= 0 && fractionDigits <= 100;
  return isNum && isValidRange ? number.toFixed(fractionDigits) : 'error';
}
...
function toPrecision(number, precision) {
  const isNum = typeof number.valueOf() === 'number';
  const isValidRange = precision >= 1 && precision <= 100;
  return isNum && isValidRange ? number.toPrecision(precision) : 'error';
}
...
function getCountOfOddNumbers(number) {
  ...
  let sum = 0;
  const numMin = Math.min(0, number);
  const numMax = Math.max(0, number);
